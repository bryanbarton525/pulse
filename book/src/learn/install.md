...
Start from the Ready `pulse-book` cluster in the previous chapter. Run commands from the Pulse repository root. This chapter installs the deterministic monitoring components first; model preparation and intelligence follow separately. There is no policy yet, so the incident engine is not needed to run the first canary.
...
The runner Dockerfile copies `hack/models/potion` if populated. A deterministic canary does not require Potion weights. Do not interpret this build as verification that models are installed or enabled. The dedicated model chapter will establish that evidence before any model experiment.
